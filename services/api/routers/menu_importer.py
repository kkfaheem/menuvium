...
async def retry_job(
    ...
    if not job:
        raise HTTPException(status_code=404, detail="Job not found")
    if job.status not in ("FAILED", "CANCELED", "NEEDS_INPUT"):
        raise HTTPException(
            status_code=400,
            detail=f"Cannot retry job with status '{job.status}'. Only FAILED, CANCELED, or NEEDS_INPUT jobs can be retried.",
        )
    job.status = "QUEUED"
    job.progress = 0
