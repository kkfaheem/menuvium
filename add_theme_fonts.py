#!/usr/bin/env python3
"""Script to add font definitions to all menu themes."""
import re
...
# Read the file
file_path = "apps/web/src/lib/menuThemes.ts"
with open(file_path, "r") as f:
    content = f.read()

# Track which themes we've processed
added = 0
skipped = 0

for theme_id, fonts in THEME_FONTS.items():
    # Use a precise pattern to find the layout line for this theme
    # Match: id: "theme_id", ... layout: "xxx",
    # And capture up to but not including palette
    pattern = rf'(id: "{theme_id}",\n[^\{{]*?layout: "(?:noir|paper|citrus|harbor)",)\n(\s*)(palette:)'
    
    match = re.search(pattern, content)
    if not match:
        print(f"Could not find theme: {theme_id}")
        continue
    
    # Check if fonts already present (fonts: { appears before the next palette:)
    theme_section_start = match.start()
    palette_start = match.end()
    section = content[theme_section_start:palette_start]
    ...
    fonts_block = f'''fonts: {{
{indent}    heading: "{fonts['heading']}",
{indent}    headingWeights: "{fonts['headingWeights']}",
{indent}    body: "{fonts['body']}",
{indent}    bodyWeights: "{fonts['bodyWeights']}"
{indent}}},
{indent}'''
    
    # Replace
    replacement = f'{match.group(1)}\n{indent}{fonts_block}{match.group(3)}'
    content = content[:match.start()] + replacement + content[match.end():]
    added += 1
    print(f"Added fonts to {theme_id}")

# Write back
with open(file_path, "w") as f:
    f.write(content)

print(f"\nDone! Added fonts to {added} themes, skipped {skipped} that already had fonts.")
