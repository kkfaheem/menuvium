...
import {
  ThemeLayout,
  THEME_LAYOUT_CONFIGS,
  type ThemeLayoutConfig,
} from "@/components/public-menu/ThemeLayout";
...
export default function PublicMenuPage() {
  ...
  useEffect(() => {
    ...
    const stored =
      (localStorage.getItem("menuvium_sold_out_display") as "dim" | "hide") ||
      "dim";
  ...
  const openArExperience = (item: Item) => {
    ...
    const posterUrl =
      item.ar_model_poster_url || item.photo_url || item.photos?.[0]?.url || "";
  ...
  const filterOptions = useMemo(() => {
    ...
    Array.from(dietaryById.values()).forEach((tag) => {
      ...
      const inferred = dietDefaults.has(nameKey)
        ? "diet"
        : spiceDefaults.has(nameKey)
          ? "spice"
          : highlightDefaults.has(nameKey)
            ? "highlights"
            : "highlights";
  ...
  const itemMatchesSelectedTags = (item: Item) => {
    ...
    return selectedTagKeys.some(
      (key) => dietaryIds.has(key) || allergenIds.has(key),
    );
  };
  ...
  const fetchMenu = async () => {
    try {
      ...
      if (!res.ok) {
        ...
        if (
          normalizedDetail.includes("not active") ||
  ...
  const themeId: MenuThemeId = MENU_THEME_BY_ID[resolvedTheme]
    ? resolvedTheme
    : "noir";
  ...
  const showImages =
    previewShowImages === "1"
      ...
      : previewShowImages === "0"
        ? false
        : menu?.show_item_images !== false; // Default to true if not set
  ...
  const themeVars = {
    ...
    "--theme-heading-font": themeFonts
      ? `"${themeFonts.heading}", sans-serif`
      : "inherit",
    "--theme-body-font": themeFonts
      ? `"${themeFonts.body}", sans-serif`
      : "inherit",
  ...
  useEffect(() => {
    ...
    html.classList.add("preview-embed");
    body.classList.add("preview-embed");
    return () => {
      html.classList.remove("preview-embed");
      body.classList.remove("preview-embed");
    };
  }, [isEmbeddedPreview]);
      ...
      <div className="min-h-screen bg-black flex items-center justify-center text-white text-center p-6">
        <div className="max-w-md">
          ...
          <h1 className="text-2xl font-bold mb-2">
            {isMenuInactive ? "Menu Inactive" : "Menu Not Found"}
          </h1>
          <p className="text-white/50">
            {error || "This menu does not exist or has been removed."}
          </p>
        </div>
      </div>
    );
                      ...
                      >
                        ...
                        {item.is_sold_out && (
                          <div className="absolute inset-0 bg-black/75 flex items-center justify-center backdrop-blur-sm">
                            <span className="text-xs font-black uppercase tracking-widest rotate-[-12deg] border-2 border-red-500 px-2 py-1 text-red-400 bg-black/90 shadow-2xl">
                              Sold Out
                            </span>
                          </div>
                        )}
      ...
      <main className="max-w-md mx-auto p-4 space-y-10">
        ...
        {filteredCategories.map((category, categoryIndex) => (
          <section
            ...
            style={{
              animationDelay: `${categoryIndex * 0.08}s`,
              animationFillMode: "backwards",
            }}
          >
              ...
              {visibleItems(category.items).map((item, index) => (
                ...
                >
                  <div className="flex-1 min-w-0">
                    ...
                    <p
                      className="text-sm line-clamp-2 mt-1.5 leading-relaxed"
                      style={{ color: palette.muted }}
                    >
                      {item.description || "No description available."}
                    </p>
        ...
        {filteredCategories.map((category, categoryIndex) => (
          ...
          >
            <div className="flex items-center justify-between mb-4">
              <h2 className="text-2xl tracking-wide theme-heading">
                {category.name}
              </h2>
              <span className="text-xs" style={{ color: palette.muted }}>
                {visibleItems(category.items).length} items
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0 flex-1">
                      ...
                      <span
                        className="inline-block mt-2 text-sm font-bold px-2.5 py-1 rounded-full"
                        style={{
      ...
      >
        ...
        <div className="relative max-w-md mx-auto px-4">
          ...
          <div
            className="absolute right-0 top-0 bottom-0 w-12 pointer-events-none"
            style={{
              background: `linear-gradient(to right, transparent, ${palette.surfaceAlt})`,
            }}
          />
        </div>
      </div>
  ...
  const modalBackdrop =
    themeLayout === "noir" ? "rgba(0, 0, 0, 0.85)" : "rgba(15, 23, 42, 0.35)";
